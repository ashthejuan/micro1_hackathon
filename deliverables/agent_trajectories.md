...
*All trajectories are JSON with keys* `node, system_prompt, user_prompt, input_state, tool_calls, retries, output, human_decision, verifier_math`*. Every score in* `improvement_changelog.md` *traces to a* `verifier_math` */* `evidence_refs` *set-check in these files.*